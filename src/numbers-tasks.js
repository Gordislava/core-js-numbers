...
function getSumOfNumbers(x1, x2, x3) {
  const sum = (x1 * 10 + x2 * 10 + x3 * 10) / 10;
  return sum;
}
...
function getRandomInteger(min, max) {
  ...
  const result = Math.floor(
    Math.random() * (maxBorder - minBorder) + 1 + minBorder
  );
